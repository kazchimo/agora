...
trait TransactionsTest { self: CoinCheckExchangeImplTest.type =>
  ...
  val transactionsSuite: Spec[Any,TestFailure[Throwable],TestSuccess] = suite("#transactions")(
    testM("returns transactions") {
      ...
      assertM(testEffect.provideLayer(AsyncHttpClientZioBackend.stubLayer))(
        forall(isSubtype[CCTransaction](anything))
      )
    },
    testM("fails with Internal Server Error") {
      ...
      assertM(testEffect.provideLayer(AsyncHttpClientZioBackend.stubLayer).run)(
        fails(
